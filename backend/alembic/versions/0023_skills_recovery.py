...
def upgrade() -> None:
    ...
    op.add_column("agent_configs", sa.Column("tool_permissions", JSONB, nullable=False, server_default="[]"))
    op.add_column("agent_configs", sa.Column("max_tokens", sa.Integer(), nullable=False, server_default="2048"))
    op.add_column("agent_configs", sa.Column("temperature", sa.Integer(), nullable=False, server_default="70"))
    op.add_column("agent_configs", sa.Column("memory_type", sa.String(32), nullable=False, server_default="none"))
    op.add_column("agent_configs", sa.Column("context_window_size", sa.Integer(), nullable=False, server_default="10"))

    # ── knowledge_documents: DNA scoring ──
    op.add_column("knowledge_documents", sa.Column("confidence_score", sa.Integer(), nullable=False, server_default="50"))
    op.add_column("knowledge_documents", sa.Column("use_count", sa.Integer(), nullable=False, server_default="0"))
    op.add_column("knowledge_documents", sa.Column("positive_count", sa.Integer(), nullable=False, server_default="0"))
    op.add_column("knowledge_documents", sa.Column("negative_count", sa.Integer(), nullable=False, server_default="0"))
    ...
    op.add_column("runs", sa.Column("pause_reason", sa.String(64), nullable=False, server_default=""))
    op.add_column("runs", sa.Column("resume_policy", sa.String(32), nullable=False, server_default="auto"))
    op.add_column("runs", sa.Column("recovery_count", sa.Integer(), nullable=False, server_default="0"))
    op.add_column("runs", sa.Column("current_step_index", sa.Integer(), nullable=False, server_default="0"))

    # ── run_metrics ──
    op.create_table(
        "run_metrics",
        sa.Column("id", PG_UUID(as_uuid=True), primary_key=True),
        sa.Column("run_id", PG_UUID(as_uuid=True), sa.ForeignKey("runs.id", ondelete="CASCADE"), nullable=False, unique=True),
    ...
    op.create_table(
        ...
        sa.Column("id", PG_UUID(as_uuid=True), primary_key=True),
        sa.Column("project_id", PG_UUID(as_uuid=True), sa.ForeignKey("projects.id", ondelete="CASCADE"), nullable=False),
        sa.Column("run_id", PG_UUID(as_uuid=True), sa.ForeignKey("runs.id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("run_id", PG_UUID(as_uuid=True), sa.ForeignKey("runs.id", ondelete="CASCADE"), nullable=True),
...
def downgrade() -> None:
    ...
    for col in ("current_step_index", "recovery_count", "resume_policy", "pause_reason", "retry_after_at", "paused_at"):
    ...
    for col in ("context_window_size", "memory_type", "temperature", "max_tokens", "tool_permissions"):

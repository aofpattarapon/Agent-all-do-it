...
async def async_validate_webhook_url(
    url: str,
    allowed_schemes: frozenset[str] | None = None,
) -> str:
    """Async version of validate_webhook_url using non-blocking DNS resolution.

    Use this in async request handlers to avoid blocking the event loop during
    DNS lookups. All SSRF protection rules are identical to validate_webhook_url.
    """
    if allowed_schemes is None:
        allowed_schemes = WEBHOOK_ALLOWED_SCHEMES
    ...
    try:
        addr_infos = await loop.run_in_executor(
            None,
            functools.partial(socket.getaddrinfo, hostname, port, proto=socket.IPPROTO_TCP),
        )
    except socket.gaierror as err:
        raise SSRFBlockedError(
            f"Webhook URL blocked: unable to resolve hostname {hostname!r}"

...
class TestOpenAPISchema:
    ...
    @pytest.mark.anyio
    async def test_schema_has_project_scoped_config_endpoints(self, client: AsyncClient):
        """Secrets and integrations must be project-scoped in the contract."""
        response = await client.get("/api/v1/openapi.json")
        paths = response.json()["paths"]
        assert "/api/v1/projects/{project_id}/secrets" in paths
        assert "/api/v1/projects/{project_id}/secrets/{secret_id}" in paths
        assert "/api/v1/projects/{project_id}/integrations" in paths
        assert "/api/v1/projects/{project_id}/integrations/{integration_id}" in paths
        assert "/api/v1/secrets" not in paths
        assert "/api/v1/integrations" not in paths

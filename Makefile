...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app pixel_dream_agent db upgrade
	@echo ""
	@echo "🚀 Dev stack ready:"
	@echo "   API:      http://localhost:8100"
	@echo "   Docs:     http://localhost:8100/docs"
	@echo "   Admin:    http://localhost:8100/admin"
	@echo "   Frontend: http://localhost:3100  (run 'make dev-frontend' or 'cd frontend && bun dev')"
	@echo ""
...
seed:
	...
	@if docker compose -f docker-compose.dev.yml exec -T app \
		pixel_dream_agent user list 2>/dev/null \
...
dev-frontend:
	...
	@echo ""
	@echo "✅ Frontend at http://localhost:3100  (backend must be up — 'make dev')"

# === Staging: built images, no bind mounts (production-like, local DB) ===
stage:
	docker compose -f docker-compose.yml up -d --build
	$(call _wait_for_db,docker-compose.yml)
	docker compose -f docker-compose.yml exec -T app pixel_dream_agent db upgrade
...
prod:
	...
	@sleep 5
	docker compose --env-file backend/.env -f docker-compose.prod.yml exec -T app pixel_dream_agent db upgrade
...
db-init: docker-db
	...
	@sleep 8
	cd backend && uv run pixel_dream_agent db migrate -m "initial" || true
	cd backend && uv run pixel_dream_agent db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend pixel_dream_agent db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend pixel_dream_agent db upgrade

db-downgrade:
	uv run --directory backend pixel_dream_agent db downgrade

db-current:
	uv run --directory backend pixel_dream_agent db current

db-history:
	uv run --directory backend pixel_dream_agent db history

# === Server ===
run:
	uv run --directory backend pixel_dream_agent server run --reload

run-prod:
	uv run --directory backend pixel_dream_agent server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend pixel_dream_agent server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend pixel_dream_agent user create-admin

user-create:
	uv run --directory backend pixel_dream_agent user create

user-list:
	uv run --directory backend pixel_dream_agent user list

# === Celery ===
celery-worker:
	uv run --directory backend pixel_dream_agent celery worker

celery-beat:
	uv run --directory backend pixel_dream_agent celery beat

celery-flower:
	uv run --directory backend pixel_dream_agent celery flower
	@echo ""
	@echo "✅ Flower started at http://localhost:5556"
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5433/pixel_dream_agent"
...
help:
	@echo ""
	@echo "pixel_dream_agent - Available Commands"
	@echo "======================================"
	@echo ""

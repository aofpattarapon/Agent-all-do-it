...
export function parseSpawns(map: Phaser.Tilemaps.Tilemap) {
  ...
  // Auto-generate extra virtual seats so the room can host up to 12 agents.
  // Positions chosen to fit in walkable open-floor areas of office2.json.
  const EXTRA_SEATS: Array<{ x: number; y: number; facing: Direction }> = [
    { x: 700, y: 336, facing: "down" },
    { x: 700, y: 460, facing: "up" },
    { x: 264, y: 580, facing: "down" },
    { x: 408, y: 580, facing: "down" },
    { x: 552, y: 580, facing: "down" },
  ];
  const base = workerSpawns.length;
  for (let i = 0; i < EXTRA_SEATS.length; i++) {
    const es = EXTRA_SEATS[i];
    workerSpawns.push({ seatId: `seat-${base + i}`, x: es.x, y: es.y, facing: es.facing, index: base + i });
  }

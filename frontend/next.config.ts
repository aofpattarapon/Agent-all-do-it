...
const nextConfig: NextConfig = {
  ...
  // Prevent Next.js from bundling @opentelemetry/api into server chunks,
  // which avoids the "Cannot find module './vendor-chunks/@opentelemetry.js'"
  // error during dev and standalone builds.
  serverExternalPackages: ["@opentelemetry/api"],

  // Phaser uses browser APIs — prevent it from being bundled on the server.
  webpack: (config, { isServer }) => {
    if (isServer) {
      const externals = Array.isArray(config.externals) ? config.externals : [];
      config.externals = [...externals, "phaser"];
    }
    return config;
  },

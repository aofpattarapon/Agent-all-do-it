...
└── commands/             # Click CLI commands (auto-discovered by `pixel_dream_agent cmd …`)
...
The generated project ships a Click CLI exposed as `pixel_dream_agent` (after `make install`):

```bash
pixel_dream_agent server run --reload          # dev server
pixel_dream_agent db upgrade                   # apply migrations
pixel_dream_agent db migrate -m "message"      # create new migration
pixel_dream_agent user create-admin            # interactive admin creation
pixel_dream_agent celery worker                # start worker
pixel_dream_agent celery beat                  # start scheduler
```

Run `make help` for a categorized list, or `pixel_dream_agent --help` for full CLI docs.
...
*Generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template) v0.2.10.*
